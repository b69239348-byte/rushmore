...
def fetch_top5():
    ...
    # Mark players missing from players.json so the card generator uses name fallback
    db_path = REPO_ROOT / "players.json"
    db_ids = {p["id"] for p in json.loads(db_path.read_text())}
    for p in top5:
        p["in_db"] = p["player_id"] in db_ids
        if not p["in_db"]:
            log(f"WARNING: {p['player_name']} ({p['player_id']}) not in players.json — will use name query")
...
def main():
    ...
    cmds = [
        ["git", "-C", str(REPO_ROOT), "add", str(OUTPUT_FILE)],
